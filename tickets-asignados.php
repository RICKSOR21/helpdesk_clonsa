...
<head>
  ...
  <title>SIRA - Tickets Asignados a Mi</title>
  <!-- plugins:css -->
  ...
  <style>
    ...
    /* Badge naranja para Transfiriendo */
          ...
          <li class="nav-item">
            ...
            <div class="collapse show" id="tickets-menu">
              <ul class="nav flex-column sub-menu">
                ...
                <?php endif; ?>
                <li class="nav-item">
                  <a class="nav-link active d-flex align-items-center justify-content-between" href="tickets-asignados.php">
...
<li class="nav-item nav-category">AYUDA</li>
              ...
              <div class="card card-rounded">
                <div class="card-body">
                  <div class="d-sm-flex justify-content-between align-items-start mb-4">
                    <div>
                      <h4 class="card-title card-title-dash">Tickets Asignados a Mi</h4>
  ...
  <script>
    ...
    function loadCatalogos() {
      ...
      // En Asignados a Mi, mostrar solo tickets del usuario asignado actual.
      $('#filterCreador')
        ...
        success: function(response) {
          ...
          if(response.success) {
            ...
            allTickets = base.filter(ticket => {
              // "Asignados a Mi" = tickets transferidos a mi cuenta (aprobados)
              const esTransferidoAMi = parseInt(ticket.transferido_a_mi || 0) === 1;
              const esAsignadoActual = parseInt(ticket.asignado_a || 0) === parseInt(CURRENT_USER_ID || 0);
              // Excluir cerrados/resueltos finales (ya no vigentes en "Asignados a Mi")
              const esCerradoFinal = parseInt(ticket.estado_id || 0) === 4 && parseInt(ticket.pendiente_aprobacion || 0) === 0;
              return esTransferidoAMi && esAsignadoActual && !esCerradoFinal;
            });
    ...
    function renderTickets(tickets, totalFiltered = null) {
      ...
      tickets.forEach(ticket => {
        ...
        // Bolitas de transferencia (igual que tickets.php)
        const transferCountRaw = parseInt(ticket.transferencia_aprobada || 0);
        const showTransferBadge = transferCountRaw >= 1;
        let transferLabel = '';
        if (transferCountRaw >= 4) {
          const tDots = Array.from({length: 3}, () => '<span class="transfer-t-badge">T</span>').join('');
          transferLabel = `<span class="transfer-t-dots" title="${transferCountRaw} transferencias aprobadas">${tDots}<span class="transfer-t-badge transfer-t-badge--plus">+</span></span>`;
        } else if (transferCountRaw >= 1) {
          const tDots = Array.from({length: transferCountRaw}, () => '<span class="transfer-t-badge">T</span>').join('');
          transferLabel = `<span class="transfer-t-dots" title="${transferCountRaw} transferencia${transferCountRaw > 1 ? 's' : ''} aprobada${transferCountRaw > 1 ? 's' : ''}">${tDots}</span>`;
        }

        // Nombre principal / secundario (igual que tickets.php)
        ...
        let nombrePrincipal, nombreSecundario = null;

        const huboTransferAprobada = parseInt(ticket.transferencia_aprobada || 0) > 0;

        if (hayTransferPendiente && ticket.transfer_destino_nombre) {
          ...
          nombrePrincipal = ticket.asignado || 'Sin nombre';
          nombreSecundario = ticket.creador ? `Asignado por: ${ticket.creador.split(' ').slice(0,2).join(' ')}` : null;
        ...
        const avatarName = nombrePrincipal || 'Usuario';
        ...
        // Avatar basado en nombre principal
        const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(avatarName)}&background=667eea&color=fff&size=45`;
          ...
          <tr>
            ...
            <td>
              <div class="estado-container">
                ...
                ${showPulseOrange ? '<span class="pulse-dot-orange" title="Transferencia pendiente"></span>' : ''}
    ...
    function abrirTransferencia(ticketId) {
      ...
      $.get('api/usuarios.php?action=por_departamento&context=transfer&departamento_id=' + (ticket.departamento_id || ''), function(respUsers) {
