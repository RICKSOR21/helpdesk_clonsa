...
<head>
  ...
  <title>SIRA - Todos los Tickets</title>
  <!-- plugins:css -->
  ...
  <style>
    ...
    .transfer-t-badge--plus {
      background: #1F3BB3;
      font-size: 13px;
      box-shadow: 0 0 0 0 rgba(31, 59, 179, 0.7);
      animation: transfer-blink-blue 1.05s infinite;
    }
    @keyframes transfer-blink-blue {
      0%, 100% { opacity: 1; box-shadow: 0 0 0 0 rgba(31, 59, 179, 0.7); }
      50%       { opacity: .35; box-shadow: 0 0 0 6px rgba(31, 59, 179, 0); }
    }
          ...
          <li class="nav-item">
            ...
            <div class="collapse show" id="tickets-menu">
              <ul class="nav flex-column sub-menu">
                <li class="nav-item">
                  <a class="nav-link active d-flex align-items-center justify-content-between" href="tickets.php">
...
<li class="nav-item nav-category">AYUDA</li>
              ...
              <div class="card card-rounded">
                <div class="card-body">
                  <div class="d-sm-flex justify-content-between align-items-start mb-4">
                    <div>
                      <h4 class="card-title card-title-dash">Lista de Tickets</h4>
  ...
  <script>
    ...
    const USER_DEPARTAMENTO = <?php echo intval($departamento_usuario); ?>;
    ...
    // Superadmin: Admin del departamento General (id=1) ve todo sin filtro
    const ES_SUPERADMIN = (USER_ROL === 'Administrador' || USER_ROL === 'Admin') && USER_DEPARTAMENTO === 1;
    const ES_JEFE_ADMIN = (USER_ROL === 'Administrador' || USER_ROL === 'Admin' || USER_ROL === 'Jefe');
    let allTickets = [];
    let filteredTickets = [];
    let currentPage = 1;
    let itemsPerPage = 10;
    ...
    function loadCatalogos() {
      // Superadmin ve todos los catálogos sin filtro; Admin/Jefe de área filtran por su dpto
      const deptParam = ES_SUPERADMIN
        ? ''
        : (USER_ROL === 'Administrador' || USER_ROL === 'Admin' || USER_ROL === 'Jefe')
          ? `&departamento_id=${USER_DEPARTAMENTO}`
          : `&departamento_id=${USER_DEPARTAMENTO}`;

      // Estados (siempre todos)
      ...
      // Actividades — filtradas por departamento si corresponde
      $.get('api/catalogos.php?tipo=actividades' + deptParam, function(response) {
        if(response.success) {
      ...
      // Ubicaciones — filtradas por departamento si corresponde
      $.get('api/catalogos.php?tipo=ubicaciones' + deptParam, function(response) {
        if(response.success) {
          response.data.forEach(item => {
      ...
      // Equipos — filtrados por departamento si corresponde
      $.get('api/catalogos.php?tipo=equipos' + deptParam, function(response) {
        if(response.success) {
          response.data.forEach(item => {
      ...
      // Usuarios (para filtro de creador)
      if (!ES_JEFE_ADMIN) {
        // Usuario normal: solo su nombre, deshabilitado
        // value="" para no interferir con el filtro (la API ya filtra por usuario)
        $('#filterCreador')
          .html(`<option value="">${CURRENT_USER_NAME}</option>`)
          .val('')
          .prop('disabled', true);
      } else {
        // Admin/Jefe: cargar todos los usuarios del departamento
        const usuariosUrl = deptParam
          ? `api/catalogos.php?tipo=usuarios${deptParam}`
          : 'api/catalogos.php?tipo=usuarios';
        $.get(usuariosUrl, function(response) {
          if(response.success) {
            response.data.forEach(item => {
              $('#filterCreador').append(`<option value="${item.id}">${item.nombre_completo || item.nombre}</option>`);
            });
          }
        });
      }
    ...
    function renderTickets(tickets, totalFiltered = null) {
      ...
      tickets.forEach(ticket => {
        ...
        let showPulseOrange = false;
        let showCheckIcon = false;
        const transferCountRaw = parseInt(ticket.transferencia_aprobada || 0);
        const showTransferBadge = transferCountRaw >= 1;
        let transferLabel = '';
        if (transferCountRaw >= 1) {
          const tCount = Math.min(transferCountRaw, 3);
          const transferDots = Array.from({length: tCount}, () => '<span class="transfer-t-badge">T</span>').join('');
          const plusDot = transferCountRaw >= 4 ? '<span class="transfer-t-badge transfer-t-badge--plus">+</span>' : '';
          const titleText = transferCountRaw > 1 ? `${transferCountRaw} transferencias aprobadas` : '1 transferencia aprobada';
          transferLabel = `<span class="transfer-t-dots" title="${titleText}">${transferDots}${plusDot}</span>`;
        }
        const hayTransferPendiente = parseInt(ticket.transferencia_pendiente || 0) === 1;
        ...
        if(hayTransferPendiente) {
          // Transfiriendo - Naranja parpadeante (máxima prioridad visual)
          estadoBadgeClass = 'badge-opacity-transferring';
          showPulseOrange = true;
          estadoDisplay = 'Transfiriendo';
        } else if(ticket.estado_id == 5) {
          // Rechazado - Rojo
        ...
        // Usar campos originales de la tabla para comparar IDs
        const creadorId  = parseInt(ticket.usuario_id  || 0);
        const asignadoId = parseInt(ticket.asignado_a  || 0);
        const tieneAsignado   = asignadoId > 0;
        const asignadoPorOtro = tieneAsignado && asignadoId !== creadorId;

        // Si hay transferencia pendiente: mostrar destino provisional
        const hayTransferPend = parseInt(ticket.transferencia_pendiente || 0) === 1;
        let nombrePrincipal, nombreSecundario;

        const huboTransferAprobada = parseInt(ticket.transferencia_aprobada || 0) > 0;

        if (hayTransferPend && ticket.transfer_destino_nombre) {
          // Transferencia pendiente: destino provisional + quién la solicitó
          nombrePrincipal = ticket.transfer_destino_nombre;
          const solicitante = ticket.transfer_solicitado_por_nombre
            ? ticket.transfer_solicitado_por_nombre.split(' ').slice(0,2).join(' ')
            : (ticket.asignado ? ticket.asignado.split(' ').slice(0,2).join(' ') : null);
          nombreSecundario = solicitante ? `Transferido por: ${solicitante}` : null;
        } else if (huboTransferAprobada && ticket.ultima_transfer_origen_nombre) {
          // Transferencia aprobada: asignado actual + de quién provino la transferencia
          nombrePrincipal = ticket.asignado || 'Sin nombre';
          const origen = ticket.ultima_transfer_origen_nombre.split(' ').slice(0,2).join(' ');
          nombreSecundario = `Transferido por: ${origen}`;
        } else if (asignadoPorOtro) {
          // Asignado directamente por jefe/admin
          nombrePrincipal  = ticket.asignado || 'Sin nombre';
          nombreSecundario = ticket.creador
            ? `Asignado por: ${ticket.creador.split(' ').slice(0,2).join(' ')}`
            : null;
          ...
          // Sin asignar, mostrar creador
          nombrePrincipal  = ticket.creador || 'Sin usuario';
          nombreSecundario = null;
        }
        ...
        html += `
          <tr>
              ...
              <div class="d-flex">
                ...
                <div>
                  <h6 class="mb-0 d-flex align-items-center gap-2">
                    <span>${nombrePrincipal}</span>
                    ${showTransferBadge ? transferLabel : ''}
                  </h6>
                  <p class="text-muted mb-0" style="font-size: 12px;">
                    ${formatDate(ticket.created_at)}
                    ${nombreSecundario ? `<br><span style="font-size: 11px; color: #999;">${nombreSecundario}</span>` : ''}
                  </p>
                </div>
              </div>
            </td>
            <td>
              ${(() => {
                const dIds = (ticket.departamentos_actividad_ids || '').split(',').filter(Boolean);
            ...
            <td>
              <div class="estado-container">
                ...
                ${showPulseOrange? '<span class="pulse-dot-orange"  title="Transferencia pendiente de aprobacion"></span>' : ''}
    ...
    function abrirTransferencia(ticketId) {
      ...
      // Si la actividad es compartida (múltiples departamentos), usar actividad_id para traer todos
      const deptIds = (ticket.departamentos_actividad_ids || '').split(',').filter(Boolean);
      const esActividadCompartida = deptIds.length > 1;
      const urlUsuarios = esActividadCompartida
        ? 'api/usuarios.php?action=por_departamento&context=transfer&actividad_id=' + (ticket.actividad_id || '')
        : 'api/usuarios.php?action=por_departamento&context=transfer&departamento_id=' + (ticket.departamento_id || '');

      $.get(urlUsuarios, function(respUsers) {
        if(respUsers.success) {
          ...
          (respUsers.data || []).forEach(u => {
            ...
            // Si hay múltiples departamentos, agrupar con optgroup
            if(esActividadCompartida && u.departamento_nombre && u.departamento_nombre !== lastDept) {
              if(lastDept !== null) htmlUsers += '</optgroup>';
              htmlUsers += `<optgroup label="${u.departamento_nombre}">`;
              lastDept = u.departamento_nombre;
            }
            htmlUsers += `<option value="${u.id}">${u.nombre_completo}</option>`;
          });
          if(esActividadCompartida && lastDept !== null) htmlUsers += '</optgroup>';
